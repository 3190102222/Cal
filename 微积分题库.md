...
> [!Question]
> 设 $x \in \mathbb{R}, f(x),\varphi(x),\beta(x) \in C[a, b], \varphi(x) \geq 0$ 且 $f(x) \leq \alpha + \int_{0}^{x} [f(t)\varphi(t) + \beta(t)] \, dt, x \in [a, b]$
> 求证：$f(x) \leq \alpha e^{ \int_{0}^{x} \varphi(t) \, dt } + \int_{0}^{x} e^{ \int_{t}^{x} \varphi(s) \, ds } \beta(t) \, dt$
